...
if ($status === 'missing_fields') {
  $notice = 'Completeaza toate campurile pentru inregistrare.';
  $notice_class = 'error';
} elseif ($status === 'invalid_email') {
  $notice = 'Email invalid. Verifica formatul.';
  $notice_class = 'error';
} elseif ($status === 'password_too_short') {
  $notice = 'Parola trebuie sa aiba cel putin 6 caractere.';
  $notice_class = 'error';
} elseif ($status === 'password_mismatch') {
  $notice = 'Parolele nu se potrivesc.';
  $notice_class = 'error';
} elseif ($status === 'email_exists') {
  $notice = 'Exista deja un cont cu acest email.';
  $notice_class = 'error';
} elseif ($status === 'save_failed') {
  $notice = 'Nu am putut salva contul. Incearca din nou.';
  $notice_class = 'error';
...
<head>
  ...
  <title><?= htmlspecialchars(platform_name()) ?> - Inregistrare</title>
  ...
  <div class="auth-wrapper">
    ...
    <div class="auth-card">
      <div class="auth-header">
        ...
        <h1 class="auth-title">Creeaza cont</h1>
        <p class="auth-subtitle">Alatura-te comunitatii <?= htmlspecialchars(platform_name()) ?></p>
      </div>
      ...
      <form class="auth-form" method="post" action="php/auth.php">
        ...
        <div class="form-group">
          ...
          <div class="input-wrapper">
            ...
            <input id="register-name" name="name" type="text" placeholder="Numele tau" required>
          </div>
        </div>
        ...
        <div class="form-group">
          ...
          <div class="input-wrapper">
            ...
            <input id="register-email" name="email" type="email" placeholder="nume@email.com" required>
          </div>
        </div>
        ...
        <div class="form-group">
          <label for="register-confirm">Confirma parola</label>
          <div class="input-wrapper">
            <svg class="input-icon" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
              <rect x="3" y="11" width="18" height="11" rx="2" ry="2" />
              <path d="M7 11V7a5 5 0 0110 0v4" />
            </svg>
            <input id="register-confirm" name="confirm_password" type="password" placeholder="••••••••" minlength="6" required>
          </div>
        </div>

        <button class="auth-submit" type="submit">Creeaza cont</button>
      </form>

      <div class="auth-footer">
        <p>Ai deja cont? <a class="auth-link" href="login.php">Autentifica-te</a></p>
      </div>
    </div>
